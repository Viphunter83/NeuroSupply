  ...
  <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-3 transition-colors" :class="isChanged ? 'border-blue-200 bg-blue-50/30' : ''">
    <div class="flex items-center space-x-4">
        ...
        <div class="flex-1 min-w-0">
            <h3 class="font-bold text-gray-900 text-sm leading-tight truncate">{{ item.product_name }}</h3>
            <p v-if="item.product_name_vn" class="text-[10px] text-blue-600 font-bold uppercase tracking-tight mt-0.5 truncate">{{ item.product_name_vn }}</p>
            
            <!-- AI Insight Badge -->
            <div v-if="item.insight" class="mt-1.5 flex items-start gap-1.5 p-2 bg-purple-50 rounded-lg border border-purple-100 animate-in fade-in slide-in-from-left-2 duration-500 delay-200">
                <span class="text-xs bg-purple-600 text-white px-1.5 py-0.5 rounded font-black tracking-tighter uppercase leading-none mt-0.5">AI</span>
                <p class="text-[10px] text-purple-700 font-medium leading-relaxed">{{ item.insight }}</p>
            </div>

            <p class="text-[10px] text-gray-400 mt-2 font-medium">Ожидаемо: <span class="text-gray-600">{{ item.predicted_usage || initialStock }} {{ item.unit }}</span></p>
        </div>
